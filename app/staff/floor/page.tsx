  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        {(["available", "occupied", "ready", "attention"] as TableStatus[]).map((s) => (
          <span key={s} className="text-xs font-medium text-muted-foreground">
            <span className={cn("inline-block w-3 h-3 rounded border mr-1 align-middle", STATUS_CLASSES[s])} />
            {STATUS_LABELS[s]}
          </span>
        ))}
      </div>
                    ...
                    {orderDetail.items.map((item) => (
                      <li key={item.id} className="flex justify-between text-sm">
                        <span>
                          ...
                          {item.notes && (
                            <span className="text-zinc-500 italic"> ({item.notes})</span>
                          )}
                        </span>
